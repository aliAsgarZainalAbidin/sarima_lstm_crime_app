    ...
    with gr.Tabs() as main_tabs:
        with gr.TabItem("", id="home"):
            ...
            coords_file = gr.State(
                value="src\crime_forecast\lokasi_kejahatan_gowa_with_coords_20251007_220421 (1).csv"
            )
            geojson_file = gr.State(
                value="src\crime_forecast\lokasi_kejahatan_gowa_POINTS_20251007_220421.geojson"
            )
        ...
        with gr.TabItem("", id="analysis"):
            ...
            gr.Markdown(
                "Catatan: Parameter pelatihan di bawah ini sekarang diatur secara statis menggunakan `gr.State` dan tidak dapat diubah dari UI. Untuk mengubahnya, edit kode sumber."
            )

            with gr.Row():
                gr.Number(
                    value=test_len_state.value,
                    interactive=False,
                    label="Panjang Test (bulan)",
                )
                gr.Number(
                    value=horizon_state.value,
                    interactive=False,
                    label="Horizon Forecast (bulan)",
                )

            gr.Markdown("**SARIMA**")
            with gr.Row():
                gr.Checkbox(
                    value=use_auto_state.value,
                    interactive=False,
                    label="Gunakan auto_arima (pmdarima)",
                )
            ...
            with gr.Row():
                gr.Textbox(
                    value=grid_win_state.value,
                    interactive=False,
                    label="Coba Window (comma-separated)",
                )
                gr.Textbox(
                    value=grid_hid_state.value,
                    interactive=False,
                    label="Coba Hidden Units (comma-separated)",
                )
            with gr.Row():
                gr.Number(
                    value=lr_state.value, interactive=False, label="Learning Rate"
                )
                gr.Number(
                    value=batch_size_state.value, interactive=False, label="Batch Size"
                )
                gr.Number(value=epochs_state.value, interactive=False, label="Epochs")
                gr.Number(
                    value=patience_state.value, interactive=False, label="Patience"
                )
            ...
            with gr.Row():
                ...
                peak_tbl = gr.Dataframe(
                    label="3 Bulan Puncak (Rata-rata Tertinggi)",
                    interactive=False,
                    type="pandas",
                )
            ...
            table_hist = gr.Dataframe(
                label="Data Historis (setelah praproses)", interactive=False
            )
            ...
            with gr.Row():
                ...
                pred_range_output = gr.Dataframe(
                    label="Hasil Prediksi Jumlah Kejahatan", interactive=False
                )
