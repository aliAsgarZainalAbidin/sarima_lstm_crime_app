...
def _prepare_pipeline_inputs(
    db_data, date_c, value_c, do_out, waktu_c, tkp_c, jenis_c, jumlah_c,
...
def run_analysis_pipeline(
    ...
    try:
        ...
        print("Memulai pipeline analisis dengan argumen:", locals())
        ...
        error_html = f"<div style='color:red; padding: 12px;'>Terjadi Kesalahan: {e}</div>" # For HTML output
        
        # Mengembalikan tuple dengan ukuran yang benar sesuai dengan 'outputs' pada run_btn.click
        outputs = [
            error_fig, error_fig, error_fig, error_fig, error_fig, # plots
            None, None, # tables
            error_fig, error_fig, error_fig, error_fig, # more plots
            error_html, None # html and final table
        ]
        
        return tuple(outputs)

def save_new_event(waktu, jenis, tkp):
    """Menyimpan data kejadian baru ke database SQLite lokal."""
    if not waktu or not jenis or not tkp:
    ...
    try:
        ...
        if 'jumlah_kejadian' not in columns:
            cursor.execute(f'ALTER TABLE {table_name} ADD COLUMN jumlah_kejadian INTEGER NOT NULL DEFAULT 1')
            conn.commit()
        ...
        cursor.execute(f'INSERT INTO {table_name} (waktu_kejadian, jenis_kejahatan, tkp) VALUES (?, ?, ?)', (waktu_str, jenis, tkp))
        conn.commit()
...
def import_data_to_db(file):
    ...
    if 'jumlah_kejadian' not in df.columns:
        ...
        df['jumlah_kejadian'] = pd.to_numeric(df['jumlah_kejadian'], errors='coerce').fillna(1).astype(int)

    conn = None
    try:
        conn = sqlite3.connect("database_kejadian.db")
        save_new_event(None, None, None) # Ensure table exists
        df_to_insert = df[['waktu_kejadian', 'jenis_kejahatan', 'tkp', 'jumlah_kejadian']]
        df_to_insert['waktu_kejadian'] = pd.to_datetime(df_to_insert['waktu_kejadian']).dt.strftime('%Y-%m-%dT%H:%M:%S')
...
def load_data_from_db():
    ...
    try:
        conn = sqlite3.connect("database_kejadian.db")
        cursor = conn.cursor()
        cursor.execute("SELECT name FROM sqlite_master WHERE type='table' AND name='kejadian'")
        if cursor.fetchone() is None:
            return pd.DataFrame(columns=['id', 'waktu_kejadian', 'jenis_kejahatan', 'tkp', 'jumlah_kejadian'])
        return pd.read_sql_query("SELECT * FROM kejadian", conn)
    except Exception as e:
