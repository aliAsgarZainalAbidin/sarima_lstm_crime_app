...
def ensure_monthly_sum(df, date_col, value_col):
    ...
    if (date_col in df.columns) and (value_col in df.columns):
        ...
        date_candidates = [k for k, v in col_map.items() if v in
                           ["tanggal", "tgl", "date", "time", "waktu", "bulan", "month", "period", "periode", "waktu_kejadian"]]
        value_candidates = [k for k, v in col_map.items() if v in
                            ["jumlah", "nilai", "value", "count", "kejahatan", "kasus", "cases", "y", "total", "jumlah_kejadian"]]
        ...
        if sel_date is None:
            sel_date = next((c for c in df.columns if pd.api.types.is_datetime64_any_dtype(df[c]) or pd.api.types.is_object_dtype(df[c]) and pd.to_datetime(df[c], errors="coerce").notna().any()), None)
        if sel_value is None:
            sel_value = next((c for c in df.columns if pd.api.types.is_numeric_dtype(df[c])), None)

    if sel_date is None or sel_value is None:
        raise ValueError("Tidak dapat menemukan kolom tanggal/nilai. Periksa nama kolom atau isi parameter kolom secara eksplisit.")

    # pilih dan normalisasi
    out = df[[sel_date, sel_value]].rename(columns={sel_date: "date", sel_value: "value"}).copy()
    out["date"] = pd.to_datetime(out["date"], errors="coerce")
    out = out.dropna(subset=["date"])
    out = out.sort_values("date").set_index("date").resample("MS").sum().asfreq("MS").fillna(0)
    return out
