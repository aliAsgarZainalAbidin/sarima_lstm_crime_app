...
def read_coords_file(file_obj):
    ...
    try:
        ...
        lon = next((k for k, v in cols.items() if v in ["lon", "lng", "long", "longitude", "x"]), None)
...
def make_map_html(data_event, tkp_col, coords_df=None, geojson_path=None, jenis_col=None, jumlah_col=None):
    ...
    counts = (
        data_event[tkp_col].dropna().astype(str).str.strip()
        .str.lower().value_counts().reset_index()
    )
    ...
    if jenis_col and (jenis_col in data_event.columns):
        ...
        if jumlah_col and (jumlah_col in de.columns) and pd.api.types.is_numeric_dtype(de[jumlah_col]):
            grp = de.groupby(["tkp_norm", jenis_col])[jumlah_col].sum().reset_index()
            for _, row in grp.iterrows():
                breakdown_map.setdefault(row["tkp_norm"], {})[str(row[jenis_col])] = int(row[jumlah_col])
        else:
            grp = de.groupby(["tkp_norm", jenis_col]).size().reset_index(name="count")
            for _, row in grp.iterrows():
                breakdown_map.setdefault(row["tkp_norm"], {})[str(row[jenis_col])] = int(row["count"])

    if coords_df is not None and not coords_df.empty:
        mdf = counts.merge(coords_df[["tkp_norm", "lat", "lon"]], on="tkp_norm", how="inner")
        if mdf.empty:
        ...
        for _, r in mdf.iterrows():
            ...
            popup_lines = [f"<div style='font-weight:600'>{r['tkp_norm'].title()}</div>",
            ...
            if brk:
                ...
                for k, v in sorted(brk.items(), key=lambda x: -int(x[1])):
                    popup_lines.append(f"<li style='font-size:12px; margin:2px 0'>{k}: {v}</li>")
            ...
            folium.CircleMarker(
                ...
                fill=True, color="#2b6cb0", fill_opacity=0.6
            ).add_to(mc)

        HeatMap(mdf[["lat", "lon", "count"]].values.tolist(), radius=18, blur=15).add_to(m)

        if geojson_path:
            try:
                with open(geojson_path, "r", encoding="utf-8") as f:
                    gj = json.load(f)
                candidate_keys = ["name", "nama", "namobj", "kecamatan", "district", "kec", "kelurahan"]
                rows = []
                for feat in gj.get("features", []):
                    props = feat.get("properties", {})
                    key = next((k for k in candidate_keys if k in {kk.lower(): vv for kk, vv in props.items()}), None)
                    if key is None:
                ...
                if rows:
                    ...
                    for w in df_geo["wilayah"]:
                        cnt = counts[counts["tkp_norm"].str.contains(w, regex=False)]["count"].sum()
                    ...
                    if df_ch["count"].sum() > 0:
                        folium.Choropleth(
                            ...
                            key_on="feature.properties." + (
                                "name" if "name" in gj["features"][0]["properties"] else
                                next((k for k in gj["features"][0]["properties"].keys()), "name")
                            ),
