...
def infer_event_columns(
    df: pd.DataFrame, waktu_name, tkp_name, jenis_name, jumlah_name
):
    ...
    # fallback exact
    waktu = waktu or by_exact(
        "waktu_kejadian", "waktu", "tanggal", "tgl", "date", "time"
    )
...
def run_pipeline(
    ...
    if date_col is None or value_col is None:
        raise ValueError(
            "Cannot recognize date/value columns. Please fill in column names manually."
        )
    ...
    test = (
        ts_clean.iloc[-test_len:].copy() if test_len > 0 else ts_clean.iloc[0:0].copy()
    )
    ...
    # forecast test (log)
    sarima_forecast_test_t = (
        sarima_forecast(sarima_fit_t, steps=test_len)
        if test_len > 0
        ...
        for hid in gh:
            try:
                pred_res_val_t = fit_lstm_and_predict(
                    residuals_train_t.iloc[:-val_h],
                    val_h,
                    window=win,
                    hidden=hid,
                    lr=lstm_lr / 10,
                    batch_size=lstm_bs,
                    epochs=min(300, lstm_epochs + 100),
                    patience=max(10, lstm_patience),
                )
    ...
    sarima_pred_float = (
        np.expm1(sarima_forecast_test_t.values).clip(min=0)
        if test_len > 0
        else np.array([])
    )
    hybrid_pred_float = (
        np.expm1(sarima_forecast_test_t.values + pred_res_test_t).clip(min=0)
        if test_len > 0
        else np.array([])
    )
        ...
        future_pred_df = pd.DataFrame(
            {
                ...
                "Prediksi": [int(round(p)) for p in hybrid_future_t],
            }
        )
    ...
    ax.set_title(
        f"Monthly Prediction: SARIMA vs Hybrid — best (win, hidden) = {best_conf}"
    )
    ax.set_xlabel("Year")
    ...
    axes[0].boxplot(ts_clean["value"])
    axes[0].set_title("Box Plot Data")
    axes[0].set_ylabel("Number of Cases")
    axes[1].hist(ts_clean["value"], bins=20)
    axes[1].set_title("Histogram Data")
    ...
    month_rank["month_name"] = month_rank["month"].apply(
    ...
    pivot_cal = ts_all.pivot_table(
        values="value", index="year", columns="month_num", aggfunc="sum", fill_value=0
    )
    for m in range(1, 13):
        if m not in pivot_cal.columns:
            pivot_cal[m] = 0
    ...
    for i in range(pivot_cal.shape[0]):
        for j in range(12):
            axcal.text(
                j, i, int(pivot_cal.values[i, j]), ha="center", va="center", fontsize=8
            )
    plt.tight_layout()
    ...
    if (
        waktu_c
        and (waktu_c in df_raw.columns)
        and jenis_c
        and (jenis_c in df_raw.columns)
    ):
        ...
        if (
            (jumlah_c is not None)
            and (jumlah_c in data_event.columns)
            and pd.api.types.is_numeric_dtype(data_event[jumlah_c])
        ):
            agg = (
                data_event.groupby([data_event["bulan"], jenis_c])[jumlah_c]
                .sum()
                .reset_index()
            )
            qty_col = jumlah_c
            ...
            agg = (
                data_event.groupby([data_event["bulan"], jenis_c])
                .size()
                .reset_index(name="jumlah_kejd")
            )
            qty_col = "jumlah_kejd"
        ...
        if not pivot_kind.empty:
            ...
            for col in pivot_kind.columns:
                ax_pj.plot(
                    pivot_kind.index.astype(str),
                    pivot_kind[col],
                    marker="o",
                    label=str(col),
                )
            ax_pj.set_title("Number of Crimes per Month (per Type)")
            ax_pj.set_xlabel("Month")
            ax_pj.set_ylabel("Number")
            plt.xticks(rotation=45)
            ax_pj.grid(True, linestyle="--", alpha=0.5)
            ax_pj.legend(title="Type", ncols=2, fontsize=8)
            plt.tight_layout()
            ...
            ax_tb.set_xlabel("Month")
            ax_tb.set_ylabel("Number")
            plt.xticks(rotation=45)
            ax_tb.grid(True, linestyle="--", alpha=0.5)
            plt.tight_layout()
        else:
            fig_perjenis = make_info_fig("Data per-type per-month is empty.")
            fig_total_bln = make_info_fig("Total monthly data is empty.")
    else:
        fig_perjenis = make_info_fig(
            "Time/type columns not found; per-month graphs cannot be created."
        )
        fig_total_bln = make_info_fig(
            "Time/type columns not found; total monthly graphs cannot be created."
        )
    ...
    fig_loc_kind = make_info_fig(
        "Data TKP/jenis tidak memadai untuk ditampilkan pada peta."
    )
    try:
        if (
            tkp_c
            and jenis_c
            and (tkp_c in df_raw.columns)
            and (jenis_c in df_raw.columns)
        ):
            pivot_loc_kind = counts_per_location_type(
                df_raw, lokasi_col=tkp_c, jenis_col=jenis_c, jumlah_col=jumlah_c
            )
        ...
        comp_df = pd.DataFrame(
            {
                ...
                "Actual": y_true_test,
                "SARIMA": np.round(sarima_pred_float).astype(int),
                "Hybrid": np.round(hybrid_pred_float).astype(int),
            }
        )
    ...
    if not comp_df.empty:
        to_concat.append(
            comp_df.rename(columns={"Hybrid": "value"})[["date", "value"]].assign(
                kind="test_hybrid_pred"
            )
        )
    hist_csv = hist_df.copy()
    hist_csv["date"] = hist_csv["date"].dt.strftime("%Y-%m-%d")
    hist_csv = (
        hist_csv.rename(columns={"actual": "value"})
        if "actual" in hist_csv.columns
        else hist_csv
    )
    ...
    metrics_tbl = pd.DataFrame(
        {
            "SARIMA": {
                "RMSE": np.nan
                if test_len == 0
                else rmse_formula(y_true_test, sarima_pred_rounded),
                "MAE": np.nan
                if test_len == 0
                else mae_formula(y_true_test, sarima_pred_rounded),
                "R²": np.nan
                if test_len == 0
                else r2_formula(y_true_test, sarima_pred_rounded),
                "MAPE (%)": np.nan
                if test_len == 0
                else mape(y_true_test, sarima_pred_rounded),
            },
            "Hybrid_SARIMA_LSTM": {
                "RMSE": np.nan
                if test_len == 0
                else rmse_formula(y_true_test, hybrid_pred_rounded),
                "MAE": np.nan
                if test_len == 0
                else mae_formula(y_true_test, hybrid_pred_rounded),
                "R²": np.nan
                if test_len == 0
                else r2_formula(y_true_test, hybrid_pred_rounded),
                "MAPE (%)": np.nan
                if test_len == 0
                else mape(y_true_test, hybrid_pred_rounded),
            },
        }
    ).T
    ...
    kpis = {
        "rmse": None
        if test_len == 0
        else float(metrics_tbl.loc["Hybrid_SARIMA_LSTM", "RMSE"]),
        "mae": None
        if test_len == 0
        else float(metrics_tbl.loc["Hybrid_SARIMA_LSTM", "MAE"]),
        "mape": None
        if test_len == 0
        else float(metrics_tbl.loc["Hybrid_SARIMA_LSTM", "MAPE (%)"]),
        "r2": None
        if test_len == 0
        else float(metrics_tbl.loc["Hybrid_SARIMA_LSTM", "R²"]),
    }
