...
def test_sarima_forecast(sample_data):
    ...
    assert forecast.index.equals(
        pd.date_range(
            start=train_data.index[-1] + pd.DateOffset(months=1), periods=6, freq="M"
        )
    )
...
def test_fit_lstm_and_predict(sample_data):
    ...
    preds = fit_lstm_and_predict(
        residual_series, horizon, window, hidden, lr, batch_size, epochs, patience
    )
...
def test_residual_lstm_forward():
    ...
    input_tensor = torch.randn(
        1, 6, 1
    )  # Batch size of 1, sequence length of 6, feature size of 1
